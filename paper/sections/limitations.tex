...
\item \textbf{A dictionary-only score is not quite a property of the autoencoder.} Every decoder is
analysed under a declared unit-column gauge (Section~\ref{sec:dictionaries}), which removes the
scaling freedom, but not the one that matters most in practice. A dead latent --- a column whose
encoder never fires on the data --- still occupies a direction, still enters the row space, and still
changes the leverage of every other feature, while leaving the function the autoencoder computes
untouched. Two functionally identical dictionaries can therefore receive different scores here, and
the dictionaries most likely to carry such columns are the widest ones, which is the axis
Section~\ref{sec:dictionaries} varies. Resolving it needs matched activation data to define the
active support, which is exactly the input this section is built to avoid using, so it is a real
limit on the method and not only on this application of it. What the section measures is the geometry
the released matrix presents; whether that matrix has columns the model never uses is a separate
