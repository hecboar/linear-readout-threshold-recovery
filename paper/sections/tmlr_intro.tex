...
\item \textbf{Leverage yields a one-sided failure certificate, in sparsity units.} The diagnostic
also returns the smallest sparsity at which a sufficient condition guarantees that some feature is
unrecoverable by any affine rule. That certificate fires for \SaeFailEarlier{} of \SaeDicts{}
dictionaries \SaeFailGapMin{} to \SaeFailGapMax{} levels earlier than for the i.i.d.\ code of their
own shape and operating sparsity, median \SaeFailGapMedian{}, over \SaeFailRefs{} reference draws.
Because the condition is sufficient and not necessary, an earlier certificate does not establish an
earlier actual failure onset, and it does not certify that the reference is recoverable below its
own threshold. It is a worst-case statement about one feature rather than an average over all of
them, so its agreement with the ratio is not automatic --- though both are functionals of the same
leverage distribution, and neither is an external endpoint (Section~\ref{sec:discussion}).
...
what was registered before each campaign, and a register of ten defects found during this work
