...
one is worse conditioned for linear readout than a random code of its own shape, while an
...
controls' \SaeCtlCvMin{} to \SaeCtlCvMax{}. Whatever these dictionaries optimise, it is not
linear-readout cross-talk.
...
what was registered before each campaign, and a register of six defects found during this work
...
affine map can express, because the probe class contains the network's decoder; we report it for
completeness and build nothing on it. And the controlled campaigns use one task family at widths up
