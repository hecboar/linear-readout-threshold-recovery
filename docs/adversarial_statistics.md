...
"2.03 [1.77, 2.27]" (`paper/legacy/main_elsevier.tex:2530`), whose backing artifact
...
`paper/legacy/main_elsevier.tex:2229-2231`: "extrapolates to \ScPredicted{} [10.759] at d=400. We registered that
...
`paper/legacy/main_elsevier.tex:2528-2532` quotes the ReLU frontier gap "0.87 levels [0.63, 1.10]" and "2.03
...
paper advertises at `paper/legacy/main_elsevier.tex:~2361` ("every measured number in the text is a macro"). I could not
...
(`e7_stage_B.json`), and never mentioned in `paper/legacy/main_elsevier.tex` (grep finds nothing) or in any
...
`paper/legacy/main_elsevier.tex:2254` ("paired bootstrap over models gives a drop of...") and the Figure 7b caption
(`paper/legacy/main_elsevier.tex:2288-2289`, "paired bootstrap intervals over models"). The procedure
...
non-overlap of the two widths' intervals (`paper/legacy/main_elsevier.tex:2255`) is true and, since non-overlap of 95%
...
therefore blunt", `paper/legacy/main_elsevier.tex:2050-2051`); the contribution bullet at `paper/legacy/main_elsevier.tex:253-254` ("the two
...
E6 was demoted (D7, `paper/legacy/main_elsevier.tex:2296` ff.) because four points cannot discriminate growth shapes.
...
two rates cross", `paper/legacy/main_elsevier.tex:2258-2259`; "local rate rather than a law", 2233). The Figure 7a
caption does not: "Both codes' frontiers follow a power law over the four widths"
(`paper/legacy/main_elsevier.tex:2285`) is precisely the sentence E6 was punished for, and the panel's x-axis extends to
...
exactly once, for E5's nine Mann-Whitney tests (`paper/legacy/main_elsevier.tex:1853`). Nothing comparable for E7/E8.
...
- `paper/legacy/main_elsevier.tex:2231` and Figure 6 caption: E8 figure plots *medians* (`scripts/make_figures.py:365`),
...
- **Censoring.** I scanned every `s95` (network, all probe configurations, fixed readouts, native)
  ...
  — the manuscript itself declines to report (`paper/legacy/main_elsevier.tex:2530-2532`), which is the correct handling;
